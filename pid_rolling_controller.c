...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    // Initialize values //updated 12/01/22 changing initial altitude
    desired_state.roll = 0;
    desired_state.pitch = 0;
    desired_state.vx = 0;
    desired_state.vy = 0;
    desired_state.yaw_rate = 0;
    desired_state.altitude = .015;                                           //Altitude control //height of crazyflie when on top of ball?
    ...
    // Control altitude
    int key = wb_keyboard_get_key();                                        //modify for height controls //updated 12/01/22 adding w and s for changing altitude
    while (key > 0) {
      switch (key) {
        case WB_KEYBOARD_UP:
          forward_desired = +0.2;
